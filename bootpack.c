...
void HariMain(void)
{
    ...
    io_out8(PIC0_IMR, 0xf8); /* PIT と PIC1 とキーボードを許可 (11111000) */
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    for (i = 0; i < 2; i++) {
        ...
        make_testbox8(sht_cons[i], 8, 28, 240, 128, COL8_000000);
    ...
    make_testbox8(sht_win, 8, 28, 128, 16, COL8_FFFFFF);
    cursor_x = 8;
    ...
    init_mouse_cursor8(buf_mouse, 99); /* 背景色は 99 */
    /* 画面中央になるように計算 */
    mx = (binfo->scrnx - 16) / 2;
    ...
    /* 最初にキーボード状態との食い違いが内容に、設定しておくことにする */
    ...
    for (;;) {
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (256 <= i && i <= 511) { /* キーボードデータ */
                ...
                if (i != 0) {
                    timer_init(timer, &fifo, 0); /* 次は 0 を */
                    if (cursor_c >= 0) {
                        cursor_c = COL8_000000;
                    }
                } else {
                    timer_init(timer, &fifo, 1); /* 次は 1 を */
                    if (cursor_c >= 0) {
                        cursor_c = COL8_FFFFFF;
                    }
                }
...
int keywin_off(struct SHEET *key_win, struct SHEET *sht_win, int cur_c, int cur_x) {
    ...
    if (key_win == sht_win) {
        ...
        if ((key_win->flags & 0x20) != 0) {
            fifo32_put(&key_win->task->fifo, 3); /* コンソールのカーソル OFF */
        }
    }
    return cur_c;
}
...
int keywin_on(struct SHEET *key_win, struct SHEET *sht_win, int cur_c) {
    ...
    if (key_win == sht_win) {
        ...
        if ((key_win->flags & 0x20) != 0) {
            fifo32_put(&key_win->task->fifo, 2); /* コンソールのカーソル ON */
        }
    }
    return cur_c;
}
